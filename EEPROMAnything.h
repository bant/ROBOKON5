/* Copyright (C) 2013-2015 Kristian Lauszus, TKJ Electronics. All rights
 reserved.
...
template <class T>
uint16_t EEPROM_writeAnything(uint16_t addr, const T &value)
{
...
template <class T>
uint16_t EEPROM_updateAnything(uint16_t addr, const T &value)
{
    eeprom_busy_wait(); // Wait until the EEPROM is ready
    const uint8_t *p = (const uint8_t *)(const void *)&value;
    uint16_t i;
    uint16_t updateBytes;
    for (i = 0, updateBytes = 0; i < sizeof(value); i++)
    {
        // Limits number of write/erase cycles
...
template <class T>
uint16_t EEPROM_readAnything(uint16_t addr, T &value)
{
    eeprom_busy_wait(); // Wait until the EEPROM is ready
